...
# Then start the webcam
    ...
    while True:
        ...
        # loop over the face detections(harcascade algorithm implementation)
        for rect in rects:
            ...
            if is_speaking(prev_mouth_img, mouth_img,
                           debug=True):
                cv2.putText(frame, "LIP MOVEMENT", (50, 250), 1, 5, (46, 64, 83),2)
                sflag = True
                i += 1
        ...
        for face in faces:
            ...
            if blinking_ratio > 4.7:
                ebflag = True
                cv2.putText(frame, "EYE BLINKING", (50, 150), 1, 5, (0, 0, 0, 0),2)
        ...
        if sflag or ebflag or mflag:
            ...
            thread = Thread(target=play_alarm, daemon=True)  # Creates a thread for playing alarms in the background
            thread.start()  # Starts playing alarm for 3 seconds in a new thread

            # playsound("F:/Projectcode/Coma-Patient-monitoring/alarm.wav")
            if(not msg_sent):
                msg_sent=True
                fbh.store_name_and_time(patient_id,patient_name)
                send_sms('+91'+mobile_number)  # Comment while testing rest of the application
    ...
    # for a in range(0, len(time_list) - 1, 2):
    #     # df = df.concat([df, {"Start": time[a], "End": time[a + 1]}], ignore_index=True)
    #     df_list.append({'start': time_list[a], 'end': time_list[a + 1]})

    # df = pd.DataFrame(columns=['start', 'end'])

    # df = pd.concat([df, pd.DataFrame(df_list, columns=['start', 'end'])], ignore_index=True)
    # # df = pd.concat([df, pd.DataFrame(df_list, columns=['start', 'end'])], ignore_index=True)
